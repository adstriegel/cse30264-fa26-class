# Lecture 10 - Data Plane - Part 1

## Overview

At the start of the lecture, we will do a brief recap about QUIC as well as quick review of what we learned from the transport layer.  We will then discuss Coding Project 1, examining the server code and a pcap example. From there, we will then pivot into the Data Plane with a focus on the Network Layer (Layer 3) and specifically forwarding, the act of sending packets onto their destination.  This is in contrast to the Control Plane which governs the protocols on how the forwarding information is distributed / populated.

## Readings - Lecture 10

* Chapter 4 - Kurose / Ross - Data Plane
   * If you have not acquired the book, now is a great time to acquire it / switch over from Davies / Peterson
* [Video - Overview - QUIC](https://www.youtube.com/watch?v=EkVd4k0R4Tw)

## Handouts

* This Overview

## Key Points - In-Class - Lecture 10

* What is the data plane versus the control plane?
* What is the network layer?
* Compare / contrast: IPv4 vs. IPv6
* What is the difference between forwarding versus routing?
* Compare / contrast: FIB, priority queueing, WFQ
* What is a FIB?

## Upcoming Deadlines

| **Date** | **Item** | **Topic** |
|---|---|---|
| 09-21 (M) | Reading | Chapter 4 - Data Plane |
| 10-04 (Sun) | Assignment | [Coding Project 1 - Part 1](../../homework/cp1/cp1-pt1.md) |
| 09-28 (M) | Reading | Chapter 5 - Control Plane (for Wednesday)
| 10-12 (M) | Extra Credit | Study Sheet for Mid-Term Exam (due at 12 PM) |
| 10-12 (M) | Exam | Mid-Term Exam - in class |
